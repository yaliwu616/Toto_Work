# setwd("C:/Users/yawu/OneDrive - George Weston Limited-6469347-MTCAD/2019 Data Scientist Finance/IFRS 16")
 #source("C:/Users/yawu/OneDrive - George Weston Limited-6469347-MTCAD/2019 Data Scientist Finance/IFRS 16/Shiny App/ShinyApp/Bundle_Fun_IFRS16.R")
source("C:/Users/yawu/Desktop/IFRS16 Web App Package/Code/Bundle_Fun_IFRS16.R")
   ...
   title =div( "Lease Forecasting Web App", img(src="https://lcl-flip-cdn.azureedge.net/img/global/logo-footer-Loblaw_Companies_Ltd.png", 
                  ...
                  ,mainPanel(
                    ...
                    tabsetPanel(type = "tabs",
                                ...
                                tabPanel("New Leases",icon = icon("info"), br(),
                                           ...
                                           uiOutput(outputId = "downloadNewLse")
                                ...
                                tabPanel("Summary Tables", icon = icon("table"), 
                                         fluidPage(
                                           ...
                                           fluidRow( 
                                             ...
                                             column (3,
                                                     pickerInput(
                                                       ...
                                                       label = "Fiscal Year:",
                                                       choices = c(
                                                         seq(2019, max(unique(lease$Fiscal_Year)))
                                                       ),
                                          ...
                                          uiOutput(outputId = "downloadNewLseData")
                              ...
                              tabPanel("Note",icon = icon("exclamation-circle"),
                               ...
                               tabPanel("Leases Selected", icon = icon("info"), 
                                          ...
                                          # withSpinner(DT::dataTableOutput(outputId = "Leases_Selected"))
                                           ...
                                           br(),
                                           #downloadButton(outputId = "MultiChange_Leases_Selected", label = "Download")
                                           uiOutput(outputId = "downloadMulLse")
                               ...
                               tabPanel("Summary Tables", icon = icon("table"), 
                                        fluidPage(
                                          ...
                                          uiOutput(outputId = "downloadMulLseData")
              ...
              sidebarLayout(
                 ...
                 sidebarPanel( width = 3,    
                       helpText("If apply same changes to all selected leases, please use the following steps:"),
                               ...
                               textInput( inputId = "lse_remove",
                                          label = "Put Lease ID(s) need to be removed (separate by comma):",
                                          value = ''
                               ),
                               
                               numericInput( inputId = "LP_Date",
                   ...
                   tabsetPanel(type = "tabs",
                              ...
                              tabPanel("Lease Selected", icon = icon("info"),
                                         ...
                                         # downloadButton(outputId = "UniChange_Leases_Selected", label = "Download")
                                          uiOutput(outputId = "downloadUniLse")
                                       ),
                               ...
                               tabPanel("Summary Tables", icon = icon("table"), 
                                        fluidPage(
                                         ...
                                         uiOutput(outputId = "downloadUniLseData")
    ...
    tabPanel("Overall Forecasting Results",
             fluidPage(
               ...
               uiOutput(outputId = "downloadOverallLseData")
             )
    ),
    #############################################
    ### 4th tab######################################
...
workdf_exist <- eventReactive(input$unilse_submibutton, {
    ...
    if(!isTruthy(input$Lease_end_dt) == TRUE & !isTruthy(input$Legal_Entity) == TRUE &
      ...
      df <-  lease %>% filter(IFRS_END_DT >= input$Lease_end_dt[1] &
                                IFRS_END_DT <= input$Lease_end_dt[2] & !(Lease_ID %in% unlist(strsplit(input$lse_remove,",")))
                              )
      }
      ...
      df <- lease %>% filter(IFRS_END_DT >= input$Lease_end_dt[1] & IFRS_END_DT <= input$Lease_end_dt[2] &
                              as.character(Legal_Entity) %in% input$Legal_Entity & !(Lease_ID %in% unlist(strsplit(input$lse_remove,",")))
                            )
      }
      ...
      df <- lease %>% filter(IFRS_END_DT >= input$Lease_end_dt[1] & IFRS_END_DT <= input$Lease_end_dt[2] &
                              as.character(LSE_TY_DESC) %in% input$Lease_Type & !(Lease_ID %in% unlist(strsplit(input$lse_remove,",")))
                            )
      }
...
###########################################################################
  ...
  newlease_infor <- eventReactive(input$newlse_submibutton, {
    ...
    newlease2 <- sqldf("select * from newlease as a left join lease_pd_cal as b 
                        on (b.CORP_PD_STRT_DT BETWEEN a.IFRS_START_DT  AND a.IFRS_END_DT 
                        OR   a. IFRS_START_DT  BETWEEN b.CORP_PD_STRT_DT  AND b.CORP_PD_END_DT)
                      ")
  ...
  newlease_recurr_cost <- eventReactive(input$newlse_submibutton, {
   ...
   # lease_info_df <- newlease_infor()
    ...
    # lease_recur_cost2 <- sqldf(" select b.PRPTY_NUM , sum(b.monthly_pay) as monthly_pay, a.CORP_PD_STRT_DT, a.CORP_PD_END_DT 
    #                                 from lease_info_df as a inner join new_recurr_cost2 as b 
    ...
    #                             )
    #   
    # lease_recur_cost2 <- arrange(lease_recur_cost2,PRPTY_NUM,CORP_PD_STRT_DT)
    
    return(new_recurr_cost2)
...
####################################################################################################
  ...
  new_lease_table <- reactive({
    ...
    new_lease_big_data <- new_lease_big_data %>% select(Lease_ID,	PRPTY_NUM,	CO_CD,	IFRS_START_DT,	IFRS_END_DT,	ASSET_IN_SERV_DT,	INCR_BORR_RT,
                                                        ...
                                                        ) %>% rename(INTEREST_RATE = INCR_BORR_RT)
...
output$downloadUniLse <- renderUI({
  if(!isTruthy(lease.table()) == FALSE){
    downloadButton('UniChange_Leases_Selected', 'Download Output File')
  }
})  
...
output$downloadNewLse <- renderUI({
  
  if(!isTruthy(new_lease_table()) == FALSE) {
    
    downloadButton('download_data_new_lease', 'Download Output File')
  }
...
output$downloadMulLse <- renderUI({
  if(!isTruthy(multi.change.lease.table()) == FALSE) {
    
    downloadButton('MultiChange_Leases_Selected', 'Download Output File')
  }
})
       ...
       if(input$selectbreakdownby == 'Lease ID'){
         ...
         if(isTruthy(input$reporttype) == TRUE & input$reporttype == 'Period to Date'){
           ...
           unilseadd_lse <- unilseadd_lse %>% filter(reporttype == 'Quarter to Date') %>% 
                    select(-ends_with("PTD"), -ends_with("YTD"), - starts_with("PTD"), -starts_with("YTD") ) %>% 
                    mutate(Financial_Liability_Ending_Detail = -abs(Financial_Liability_Ending_Detail)
           ...
           unilseadd_lse <- unilseadd_lse %>% filter(reporttype == 'Year to Date') %>% 
                    select(-ends_with("PTD"), -ends_with("QTD"), - starts_with("PTD"), -starts_with("QTD"), 
                            -Fiscal_Quarter) %>% mutate(Financial_Liability_Ending_Detail = -abs(Financial_Liability_Ending_Detail)
          ...
          return(unilseadd_lse)
           ...
           unilseadd_lse <- unilseadd_lse %>% filter(reporttype == 'Quarter to Date') %>% 
                    select(type , Lease_ID, Legal_Entity, Fiscal_Year, Fiscal_Period, Fiscal_Quarter, Right_of_Use_Ending,  
                           ...
                           ARO_Ending, Tenant_Inducement_Ending,retained_earnings_QTD ,
                           QTD_Depn, QTD_Interest  , QTD_ARO_Interest  , QTD_TI_Amort  , QTD_Rent_Payments
                    ) %>% group_by(type, Legal_Entity, Fiscal_Year,Fiscal_Period, Fiscal_Quarter) %>% #add Fiscal_Period @5/29/2019
                    summarise( Lease_ID_Count = n_distinct(Lease_ID),
                              ...
                              ) %>% arrange(type, Legal_Entity, Fiscal_Year, as.numeric(Fiscal_Period), Fiscal_Quarter)  #%>% mutate_if(is.numeric, round, digits = 0L) %>% 
                             #mutate_if(is.numeric, formatC, digits = 0L, format="d", big.mark = ",")
...
output$downloadUniLseData <- renderUI({
  if(!isTruthy(Summary_Lease_VS_Table()) == FALSE) {
    downloadButton('download_uniChange', 'Download Output File')
  }
})
          ...
          return(newlseadd_lse)
           ...
           newlseadd_lse <- newlseadd_lse %>% filter(reporttype == 'Quarter to Date') %>% 
                    ...
                    summarise( Lease_ID_Count = n_distinct(Lease_ID),
                               ...
                               TI_Amortization_QTD = sum(QTD_TI_Amort, na.rm = T),
                               Rental_Payments_QTD = (sum(QTD_Rent_Payments, na.rm = T))
                              ) %>% arrange(type, Legal_Entity, Fiscal_Year, as.numeric(Fiscal_Period), Fiscal_Quarter)
...
output$downloadNewLseData <- renderUI({
  
  if(!isTruthy(New_Lease_VS_Table_output()) == FALSE) {
    
    downloadButton('download_data_new', 'Download Output File')
  }
})
...
output$downloadMulLseData <- renderUI({
  if(!isTruthy(multi_Summary_Lease_VS_Table()) == FALSE) {
    downloadButton('download_mulChange_mulLse', 'Download Output File')
  }
})
...
all_after_allchange <- reactive({  
  ...
  observeEvent( isTruthy(data_created),{
    
    
          if(  (any(unique(data_created$dat2$Lease_ID) %in% unique(data_created$dat3$Lease_ID))| 
                any(unique(data_created$dat1$Lease_ID) %in% unique(data_created$dat2$Lease_ID))|
  ...
  if(any(unique(data_created$dat2$Lease_ID) %in% unique(data_created$dat3$Lease_ID)) == F &     
     ...
     any(unique(data_created$dat1$Lease_ID) %in% unique(data_created$dat3$Lease_ID)) == F){
          ...
          return(DT_lease_afterallchange)}else{return(NULL)}
...
output$downloadOverallLseData <- renderUI({
  if(!isTruthy( Overall_Lease_VS_Table_df()) == FALSE){
    downloadButton('download_Overall_Change', 'Download Output File')
  }
})
