...
#            df$payment_date[rownum[j]] > df$lse_term_end_dt[rownum[j]]
#           ){
...
rownumtoremove <- function(df){
  ...
  for (i in seq_along(duplicatevalue)){
    ...
    for (j in seq_along(rownum)){
      ...
      if(isTruthy(df$payment_date[rownum[j]]) & (df$payment_date[rownum[j]] == df$lse_term_start_dt[rownum[j]] |
         (df$payment_date[rownum[j]] <= df$lse_term_end_dt[rownum[j]] & df$payment_date[rownum[j]] >= df$lse_term_start_dt[rownum[j]]
         ))){
        
       
        positionToKeep[i]  <- rownum[j]
        
      } else if (df$lse_term_start_dt[rownum[j]] >= df$CORP_PD_STRT_DT[rownum[j]] & 
                 df$lse_term_end_dt[rownum[j]] >= df$CORP_PD_END_DT[rownum[j]]
                 ){
    ...
    positionToDelete <- c(rownum[which(rownum!=positionToKeep)], positionToDelete)
...
extend_yr_period <- function(lease_end_dt, new_lease_end_dt){
  
  if(isTruthy(new_lease_end_dt) == TRUE & new_lease_end_dt > lease_end_dt){
    ...
    resultdf <- lease_pd_cal %>% filter( (CORP_YR_NUM == lease_end_year & CORP_PD_NUM > lease_end_period) | (CORP_YR_NUM > lease_end_year & CORP_YR_NUM < new_lease_end_yr)
                          | (CORP_YR_NUM == new_lease_end_yr & CORP_PD_NUM <= new_lease_end_period)) %>% select(CORP_YR_NUM, CORP_PD_NUM) %>% arrange(CORP_YR_NUM,CORP_PD_NUM )                                                           
    data.frame(startYear = resultdf$CORP_YR_NUM, Period = resultdf$CORP_PD_NUM)
  } else {data.frame()}
}
...
mon_pay_dt <- function(df, IFRSENDDT, nth_day, PO_dt){
  ...
  if(!isTruthy(PO_dt)){
    ...
    for(i in seq_along(df$Remaining_Lease_Term)){
      ...
      if(((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i]) &
        
        (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32 & ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) <= IFRSENDDT[i]){
        
        
               payment_date[i] <- ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day)
             
        
      }else if(((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i]) &
               
               (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32 & ((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) <= IFRSENDDT[i]){
           
              payment_date[i] <- ((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day)
        
      }else if(((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i]) &
               
               (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) > 31 & ((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) <= IFRSENDDT[i]){
           
              payment_date[i] <- DescTools::AddMonths(df$CORP_PD_END_DT[i]- day(df$CORP_PD_END_DT[i]) + nth_day, -1)
  ...
  return(as.Date(payment_date, origin="1970-01-01"))
    ...
    for(i in seq_along(df$Remaining_Lease_Term)){
      ...
      # print(paste(" ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day)",  ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day)))
      # print(paste("((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i])",
      # ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i])))

      # print(paste(" (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32",  (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32))

      # print(paste(" (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32",  (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32))

      # print(paste("((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) <= as.Date(PO_dt)", ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) <= as.Date(PO_dt)))
      
      if(((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i]) &
         
         (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32 & ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day) <= as.Date(PO_dt)){
        ...
        payment_date[i] <- ((df$CORP_PD_STRT_DT[i] - day(df$CORP_PD_STRT_DT[i])) + nth_day)
               ...
               (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) < 32 & ((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) <= as.Date(PO_dt)){
        
        payment_date[i] <- ((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day)
        
      }else if(((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) %between% c(df$CORP_PD_STRT_DT[i], df$CORP_PD_END_DT[i]) &
               
               (df$CORP_PD_END_DT[i] - df$CORP_PD_STRT_DT[i]) > 31 & ((df$CORP_PD_END_DT[i] - day(df$CORP_PD_END_DT[i])) + nth_day) <= as.Date(PO_dt)){
...
# nth_LP_change_ext <- function(rownum, positionUpdateInterestRate){

#   nth_LP_change <- seq(1,length(unique(rownum)))
#   nth_LP_change_ext_value <- c()
  
#   i <-  1
  
#   while( i <= length(unique(rownum))){
    
   
      
#       y <- rep(nth_LP_change[i], sum(rownum == positionUpdateInterestRate[i]))
      
    
    
#    i= i+1
   
#    nth_LP_change_ext_value <- c(nth_LP_change_ext_value, y)
      
#   }
# return(nth_LP_change_ext_value)
...
###########################################################################
# add @ 5/28/2019
rowNumByDate_PO <- function(#count_period,
                          #period_start_dt,
                          #period_end_dt,
                          PO_dt,
                          payment_date){
...
IRfun_rate <- function(position, Fiscal_Year, LP_change){
  result <- c()
  if(length(position) >1 ){
      for (i in 1:(length(position)+1)){
        if(i == 1)
        { 
          ...
          for(j in seq(position[i-1], length(Fiscal_Year))){
            ...
            result[j] <- prod((LP_change[1:i-1]/100 + 1))
          ...
          for (i in 1:length(Fiscal_Year)){
                if(i < position[1]){
                   result[i] <- 1 } else  {  result[i] <-  (LP_change[1]/100 + 1) }   
            }
  } else{ 
...
##################################################################################
IRfun_amt <- function(position, Fiscal_Year, LP_change){
  result <- c()
  if(length(position) >1 ){
      for (i in 1:(length(position)+1)){
        if(i == 1)
...
#   Lease_Payment <-  c()  
#   if( !isTruthy(addition_LP) | all(addition_LP == 0) == FALSE )  
#   {
...
# }
##############################################################################################################
LPfun_rate <- function(old_lease_payment, last_lse_pay, payment_date, nodaysinperiod, rownum, addition_LP){
  Lease_Payment <- c()  
  if( isTruthy(addition_LP) & all(addition_LP == 0) == FALSE )  
  {
    for (i in seq_along(old_lease_payment)){
       ...
       if(rownum[i] == 1 ){
          ...
          if(is.na(payment_date[i]) == TRUE){
            ...
            Lease_Payment[i] <- addition_LP[i]*(last_lse_pay)

          } else{

            Lease_Payment[i] <- 2*addition_LP[i]*(last_lse_pay)
        ...
        else if(is.na(payment_date[i]) == TRUE)
        {
          ...
          Lease_Payment[i] <-  0
             ...
             Lease_Payment[i] <- (last_lse_pay)*addition_LP[i]


        } else { 
          

              Lease_Payment[i] <- (last_lse_pay)*addition_LP[i]*2
...
#######################################################################################################################################
LPfun_amt <- function(old_lease_payment, last_lse_pay, payment_date, nodaysinperiod, rownum, addition_LP){
  Lease_Payment <-  c()  
  if( isTruthy(addition_LP) & all(addition_LP == 0) == FALSE )  
  {
    for (i in seq_along(old_lease_payment)){
    if(rownum[i] == 1 ){
          ...
          if(is.na(payment_date[i]) == TRUE){
            ...
            Lease_Payment[i] <- 2*(((last_lse_pay))+ (-addition_LP[i]))
...
#      }else if((is.na(df$payment_date[i]) == TRUE & is.na(df$lse_term_start_dt[i]) == FALSE & df$lse_term_start_dt[i] <= df$CORP_PD_STRT_DT[i])| 
#               (is.na(df$payment_date[i]) == FALSE & is.na(df$lse_term_start_dt[i]) == FALSE & df$lse_term_end_dt[i] < df$payment_date[i])|
#               (is.na(df$lse_term_start_dt[i]) == TRUE) | (is.na(df$payment_date[i]) == TRUE)
#               ){
...
new_lease_pay <- function(df){
  ...
  for(i in seq_along(df$count_period)){
     ...
     if( is.na(df$payment_date[i]) == TRUE | (is.na(df$payment_date[i]) == FALSE & df$payment_date[i] > df$IFRS_END_DT[i]) ){
...
ARO <- function(PriorAROEnding, AROinitialValue, ir, remainLeaseTerm ){
  ...
  for (i in seq_along(remainLeaseTerm)){
    if(i==1){
      AROinitialValue <- ifelse(isTruthy(AROinitialValue) == FALSE,  
                               PriorAROEnding , 
                               AROinitialValue)
  ...
  if(isTruthy(PriorROUEnding)){
      ...
      for (i in seq_along(remainLeaseTerm)){
        ...
        if(i==1){
          ...
          if( i >=  new_lse_rowNumByDate_depreci){                            
...
forecast_lease <- function(originalDF, 
  ...
  print(paste("typeof(PO_dt)", typeof(PO_dt)))
  print(paste("(PO_dt)", (PO_dt)))
  print(paste("as.Date(PO_dt)", as.Date(PO_dt)))

  print(paste("typeof(nth_payday)", typeof(nth_payday)))
  print(paste("(nth_payday)", (nth_payday)))
  print(paste("as.numeric(nth_payday)", as.numeric(nth_payday)))


  print(paste("typeof(TI_receipt_dt)", typeof(TI_receipt_dt)))
  print(paste("(TI_receipt_dt)", (TI_receipt_dt)))
  print(paste("as.Date(TI_receipt_dt)", as.Date(TI_receipt_dt)))

  print(paste("typeof(impair_dt)", typeof(impair_dt)))
  print(paste("(impair_dt)", (impair_dt)))
  print(paste("as.Date(impair_dt)", as.Date(impair_dt)))
  ...
  print(paste("unique originalDF last_lse_pay  are:", unique(originalDF$last_lse_pay))) 
...
print(paste("New_IFRS_END_DT is ", New_IFRS_END_DT))
# for leases with no more valid renew option,  unique(new_lse_end_dt$new_end_dt) = 0 
 ...
 # print(paste("originalDF new IFRS end date is ", unique(originalDF$New_IFRS_END_DATE)))
  ...
  lease_add_yr_num <- round(as.numeric(difftime(New_IFRS_END_DT, unique(originalDF$IFRS_END_DT), units = 'weeks'))/52.25, digits = 2L)
  ...
  print(paste(" last(originalDF$IFRS_END_DT) IS", last(originalDF$IFRS_END_DT)))
  print(paste(" unique(New_IFRS_END_DT) IS", unique(New_IFRS_END_DT)))
  # extendYrPeriod <- extend_yr_period(last(originalDF$IFRS_END_DT), 
  #                                     unique(originalDF$New_IFRS_END_DATE))
  
  extendYrPeriod <- extend_yr_period(last(originalDF$IFRS_END_DT), 
                                      unique(New_IFRS_END_DT))
  ...
  #extenddf$last_lse_pay <- unique(originalDF$last_lse_pay)  # add @ 5/28/2019
  ...
  print(paste("newIR(inputNewIR, remain) is ", newIR(inputNewIR, remain)))
  ...
  #print(paste("glimpse(newdf2) is", glimpse(newdf2)))

  #newdf2$payment_date <-  mon_pay_dt(newdf2, newdf2$New_IFRS_END_DATE, nth_payday, PO_dt)
  New_IFRS_END_DT_column <- rep(New_IFRS_END_DT, nrow(newdf2))
  newdf2$payment_date <-  mon_pay_dt(newdf2, New_IFRS_END_DT_column, nth_payday, PO_dt)
  print(paste("last(newdf2$payment_date)", last(newdf2$payment_date)))
 ...
 # create a vector or add year 
  ...
  #             c(lease_add_yr_num)  # less than 5 years 
              
  #           } else if(lease_add_yr_num %% 5 >0 ){
              
  #             c(rep(5, lease_add_yr_num %/% 5), lease_add_yr_num %% 5) # more than 5 years but not x times of 5
              
  #           } else{

  #             c(rep(5, lease_add_yr_num %/% 5)) # more than 5 years & it is x times of 5

  #                 }

 addyear <- if (lease_add_yr_num>0 & lease_add_yr_num %/% 5 ==0) {
  
              c(lease_add_yr_num)  # less than 5 years 
              
            } else if(lease_add_yr_num>0 & lease_add_yr_num %% 5 >0 ){
              
              c(rep(5, lease_add_yr_num %/% 5), lease_add_yr_num %% 5) # more than 5 years but not x times of 5
              
            } else if (lease_add_yr_num>0){
  ...
  print(paste("addyear is", addyear))
  ...
  #num_times_LP_updated <- nth_LP_change_ext(rownum, positionUpdateInterestRate)

  # add_LP <- IRfun(positionUpdateInterestRate, newdf3$Fiscal_Year, LP_change_value)

  add_LP <- if(isTruthy(IR_for_LP) & !isTruthy(amt_for_lp)){
    
              IRfun_rate(positionUpdateInterestRate, newdf3$Fiscal_Year, LP_change_value)
            }else{
             IRfun_amt(positionUpdateInterestRate, newdf3$Fiscal_Year, LP_change_value)
            }
  print(paste("unique add_LP are ", unique(add_LP)))
# print(paste("type of add_LP are ", typeof(add_LP)))

 print(paste("-unique(change$last_lse_pay) ", -unique(change$last_lse_pay)))
 print(paste("unique(newdf3$Financial_Liability_Lease_Payment)", unique(newdf3$Financial_Liability_Lease_Payment)))
 print(paste("unique(change$Financial_Liability_Lease_Payment)", unique(change$Financial_Liability_Lease_Payment)))
newdf3$Lease_Payment <- if(isTruthy(IR_for_LP) & !isTruthy(amt_for_lp)){
  
                          LPfun_rate(newdf3$Financial_Liability_Lease_Payment, -unique(change$last_lse_pay),
                                                    newdf3$payment_date,
                                                    newdf3$nodaysinperiod, rownum ,add_LP)
                                                    }else if(!isTruthy(IR_for_LP) & isTruthy(amt_for_lp)){ 
                          
                          
                          LPfun_amt(newdf3$Financial_Liability_Lease_Payment, -unique(change$last_lse_pay),
                                                                  newdf3$payment_date,
                                                                  newdf3$nodaysinperiod, rownum ,add_LP)                         
  }else{

         newdf3$Financial_Liability_Lease_Payment  
  ...
  newdf3$PO_amt <- AdjAmtCal(newdf3$count_period,  rowNumByDate_PO( 
                                                                PO_dt,
                                                                newdf3$payment_date), 
                                                                PO_amt)

  print(paste("unique PO_amt is", unique(newdf3$PO_amt)))
  ...
  print(paste("last newdf3$PV is", last(newdf3$PV)))
  ...
  print(paste("update liability sum are:", map(Update_Liability,sum, na.rm = TRUE)))
  ...
  ##########################################################
  remain$IFRS_END_DT <- New_IFRS_END_DT # update @ 5/28/2019
  ...
  newdf3$last_lse_pay  <- remain$last_lse_pay[1]
  ###########################################################
...
recurring_cost_df <- sqldf("

select b.PRPTY_NUM , sum(b.monthly_pay) as monthly_pay, a.CORP_PD_STRT_DT, a.CORP_PD_END_DT 
from lease_info_df as a inner join recurring_cost_df as b 
on a.payment_date between b.lse_term_start_dt and b.lse_term_end_dt
group by b.PRPTY_NUM, a.CORP_PD_STRT_DT, a.CORP_PD_END_DT
"
)
...
combine_df_2$Financial_Liability_Lease_Payment <-  new_lease_pay(combine_df_2)  #new_lease_pay(combine_df_2, min_pay_term_dt)
...
# PO 
combine_df_2$PO_amt <- AdjAmtCal(combine_df_2$count_period,  rowNumByDate_PO(unique(combine_df_2$PO_Date),
                                                                              combine_df_2$payment_date), 
